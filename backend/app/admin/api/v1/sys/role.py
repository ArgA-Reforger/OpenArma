...
@router.get('/{pk}/scopes', summary='Get all data scopes of a role', dependencies=[DependsJwtAuth])
...
@router.get('/{pk}', summary='Get role detail', dependencies=[DependsJwtAuth])
async def get_role(
    db: CurrentSession, pk: Annotated[int, Path(description='Role ID')]
...
@router.put(
    '/{pk}/scopes',
    summary='Update role data scope',
    dependencies=[
