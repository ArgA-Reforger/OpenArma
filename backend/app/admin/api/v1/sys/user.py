...
@router.get('/{pk}/roles', summary='Get all roles of a user', dependencies=[DependsJwtAuth])
...
@router.put('/{pk}', summary='Update user info', dependencies=[DependsSuperUser])
...
@router.put('/me/password', summary="Update the current user's password", dependencies=[DependsJwtAuth])
...
@router.put('/me/nickname', summary="Update the current user's nickname", dependencies=[DependsJwtAuth])
async def update_user_nickname(
    db: CurrentSessionTransaction,
    request: Request,
    nickname: Annotated[str, Body(embed=True, description='Nickname')],
) -> ResponseModel:
...
@router.put('/me/avatar', summary="Update the current user's avatar", dependencies=[DependsJwtAuth])
...
@router.put('/me/email', summary="Update the current user's email", dependencies=[DependsJwtAuth])
async def update_user_email(
    db: CurrentSessionTransaction,
    request: Request,
    captcha: Annotated[str, Body(embed=True, description='Email verification code')],
    email: Annotated[str, Body(embed=True, description='User email address')],
) -> ResponseModel:
