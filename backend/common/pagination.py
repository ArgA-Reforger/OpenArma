...
class _PageDetails(BaseModel):
    ...
    items: list = Field([], description='Data list for the current page')
...
class PageData(_PageDetails, Generic[SchemaT]):
    """
    Unified response model that includes the returned data schema, only for pagination endpoints
