...
class _TaskRunner:
    """Task runner that runs an asyncio event loop on a background thread"""
...
def run_await(coro: Callable[..., Awaitable[T]] | Callable[..., Coroutine[Any, Any, T]]) -> Callable[..., T]:
    """Wrap a coroutine in a function until it finishes executing"""
    ...
    def wrapped(*args, **kwargs):  # noqa: ANN202
        ...
        try:
            # If the event loop is already running, call it through the task runner
