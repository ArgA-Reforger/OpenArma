...
class MsgSpecJSONResponse(JSONResponse):
    """
    Response class that uses the high-performance msgspec library to serialize data to JSON
    """
...
def select_columns_serialize(row: R) -> dict[str, Any]:
    """
    Serialize the columns of a SQLAlchemy query table, excluding related columns
...
def select_as_dict(row: R, *, use_alias: bool = False) -> dict[str, Any]:
    """
    Convert a SQLAlchemy query result to a dict, which may include related data

    :param row: SQLAlchemy query result row
    :param use_alias: whether to use the alias as the column name
    :return:
    """
    if not use_alias:
        result = row.__dict__
...
def select_join_serialize(  # noqa: C901
    ...
    *,
    ...
    """
    Serialize a SQLAlchemy join query result to a dict or namedtuple

    Flat serialization (relationships=None):
        All results are flattened to the same level, without nesting
        Example: Result(name='Alice', dept=Dept(...))

    Nested serialization:
        Organizes data in a nested structure based on the relationship type, supporting hierarchical structures
    ...
    Relationship format: source_model-type-target_model[:custom_name]
        ...
        - m2o/o2o: the target field name stays singular
    ...
    :param return_as_dict: True returns a dict, False returns a namedtuple
    :return:
    """
