...
@router.get('/all', summary='Get all dict data', dependencies=[DependsJwtAuth])
...
@router.get('/{pk}', summary='Get dict data detail', dependencies=[DependsJwtAuth])
async def get_dict_data(
    db: CurrentSession,
    pk: Annotated[int, Path(description='Dict data ID')],
) -> ResponseSchemaModel[GetDictDataDetail]:
    data = await dict_data_service.get(db=db, pk=pk)
    return response_base.success(data=data)


@router.get('/type-codes/{code}', summary='Get dict data list', dependencies=[DependsJwtAuth])
async def get_dict_data_by_type_code(
    db: CurrentSession,
    code: Annotated[str, Path(description='Dict type code')],
...
@router.get(
    '',
    summary='Get all dict data, paginated',
    dependencies=[
        DependsJwtAuth,
        DependsPagination,
    ],
...
async def get_dict_datas_paginated(
    db: CurrentSession,
    type_code: Annotated[str | None, Query(description='Dict type code')] = None,
    label: Annotated[str | None, Query(description='Dict data label')] = None,
    value: Annotated[str | None, Query(description='Dict data value')] = None,
    status: Annotated[int | None, Query(description='Status')] = None,
    type_id: Annotated[int | None, Query(description='Dict type ID')] = None,
...
@router.delete(
    '',
    summary='Batch delete dict data',
    dependencies=[
